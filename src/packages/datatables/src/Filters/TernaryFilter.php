...
class TernaryFilter extends Filter
{
    ...
    {
        $this->queries = [
            'yes' => fn (Builder $query) => $this->applyToColumns($query, fn (Builder $query, string $column) => $query->where($column, true)),
            'no' => fn (Builder $query) => $this->applyToColumns($query, fn (Builder $query, string $column) => $query->where($column, false)),
            'empty' => fn (Builder $query) => $this->applyToColumns($query, fn (Builder $query, string $column) => $query->whereNull($column)),
        ];
